#!/usr/bin/env python3
"""
Language utilities for Android Resource Translator.

This module provides utilities for handling language codes and names,
particularly for mapping between Android resource language codes and
their full human-readable names.

Obtained with the following code
println("LANGUAGE_CODES = {")
println("    \"default\": \"Default (English)\",")

// Track processed language+country combinations to avoid duplicates
val processedCodes = mutableSetOf<String>()

for (locale in availableLocales) {
    // Create language code or language-country code
    val langCode = if (locale.country.isNotEmpty()) {
        "${locale.language}-${locale.country.lowercase()}"
    } else {
        locale.language
    }

    // Skip duplicates
    if (!processedCodes.contains(langCode) && langCode.isNotEmpty()) {
        processedCodes.add(langCode)

        // Create display name with region information when country is present
        val displayName = if (locale.country.isNotEmpty()) {
            "${locale.displayLanguage} (${locale.displayCountry})"
        } else {
            locale.displayLanguage
        }

        println("    \"$langCode\": \"$displayName\",")
    }
}
...
LOCALE_MAPPINGS = {
    ...
    "yue-Hans-CN": "Cantonese (Simplified Han,China)",
    "yue-Hant": "Cantonese (Traditional Han)",
    "yue-Hant-HK": "Cantonese (Traditional Han,Hong Kong)",
...
def get_language_name(locale_code: str) -> str:
    """
    Get locale display name from various formats:
    - Language tag (e.g., 'pt-PT')
    - Android standard resource qualifier (e.g., 'values-pt-rPT')
    - Android BCP 47 resource qualifier (e.g., 'values-b+pt+PT')
    
    Returns the display name or the original code if not found.
    """
    # Direct match
    if locale_code in LOCALE_MAPPINGS:
        return LOCALE_MAPPINGS[locale_code]
    
    # If not found, try to normalize the format
    
    # If it's a standard Android format (pt-rPT), convert to language tag
    if "-r" in locale_code and not locale_code.startswith("b+"):
        parts = locale_code.split("-r")
        if len(parts) == 2:
            language_tag = f"{parts[0]}-{parts[1]}"
            if language_tag in LOCALE_MAPPINGS:
                return LOCALE_MAPPINGS[language_tag]
    
    # If it's a BCP 47 format (b+pt+PT), convert to language tag
    if locale_code.startswith("b+"):
        language_tag = locale_code[2:].replace("+", "-")
        if language_tag in LOCALE_MAPPINGS:
            return LOCALE_MAPPINGS[language_tag]
    
    logger.warning(f"Language name not found for code: {locale_code}")

    # Return the original code for unknown languages
    return locale_code