...
from git_utils import parse_gitignore, is_ignored_by_gitignore, find_all_gitignores
...
"""
    ...
    def test_is_ignored_by_gitignore(self):
        ...
        not_ignored_paths = [
            Path(
                "src/main/java/com/example/BuildConfig.java"
            ),  # "build" only as substring
        ...
        for path in ignored_paths:
            path_str = str(path).replace(
                "\\", "/"
            )  # Normalize path separators for platform independence
            self.assertTrue(
    ...
    def test_nonexistent_gitignore(self):
        ...
        self.assertEqual(
            patterns, [], "Empty list expected for non-existent .gitignore"
        )
...
"""
    ...
    def test_negation_patterns(self):
        ...
        patterns = ["/build/", "*.log", "!important.log", "temp/", "!temp/keep.txt"]
        ...
        not_ignored_paths = [Path("important.log"), Path("temp/keep.txt")]
    ...
    def test_character_classes_and_extended_glob(self):
        ...
        non_matching_paths = [
            ...
            Path(
                "image-a.gif"
            ),  # Wrong extension - expect failure due to pattern change
