...
class TestResourceFindingAndTranslation(TestIntegration):
    ...
    def setUp(self):
        ...
        module_dir = os.path.join(
            self.temp_dir.name, "test_module", "src", "main", "res"
        )
...
<resources>
    ...
    def test_find_and_translate_workflow(self, mock_translate_batch):
        ...
        log_entry = next(
            (
                e
                for e in translation_log["test_module"]["es"]["strings"]
                if e["key"] == "welcome"
            ),
            None,
        )
...
<resources>
    ...
    <plurals name="existing_plural">
        ...
        resource.plurals["new_plural"] = {"one": "1 new item", "other": "%d new items"}
